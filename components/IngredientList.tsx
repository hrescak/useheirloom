...
const IngredientList  : React.FC<IngredientListProps> = (props) => {
    const {data,error, mutate}:{data?:Partial<RecipeIngredient>[],error?:any,mutate?:any} = useSWR(`/api/recipes/${props.recipeId}/recipe-ingredients`, url => fetch(url).then(r => r.json()))
    ...
    async function onMove(ingredient) {
        ...
        await fetch(`/api/recipes/${props.recipeId}/recipe-ingredients/${ingredient.id}`, {
            method: 'POST',
            ...
            {props.editable ? (
                <>
                    ...
                    <form onSubmit={handleSubmit(onSubmit)}>
                        <div style={{display:'flex',flexDirection:'row',alignItems:'center',margin:'0.5rem 0'}}>
                            <ItemWrapper>
                                    <Plus/>
                                    <InlineInput type="text" placeholder="Add an Ingredient..." name="freeform" ref={register} />
                                    <input type="hidden" name="priority" value={highestPriority()+1} ref={register}/>
                            </ItemWrapper>        
                            <PrimaryButton onClick={handleSubmit(onSubmit)} icon={<PlusCircle/>}></PrimaryButton>            
                        </div>
                    </form>
                </>
            ) : (
