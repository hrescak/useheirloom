...
const ErrWrapper = styled.div<{separateRow?:boolean, wiggle?:boolean}>`
    ...
    animation: ${p=>p.wiggle? "shake 0.82s cubic-bezier(.36,.07,.19,.97) both" :""};
  @keyframes shake {
...
export const FormError: React.FC<{title:string, separateRow?:boolean}> = ({title, separateRow}) => {
    ...
    useEffect(()=>{
        ...
        if(prevTitle === title && title){
            ...
            setTimeout(()=>{setWiggle(false);console.log('unsetting wiggle')},1000)
        }
        if(title){
            setPrevTitle(title)
        }
    ...
    return (title && 
        <ErrWrapper separateRow={separateRow} wiggle={wiggle}>
                <Err separateRow={separateRow}><AlertTriangle size={16} style={{verticalAlign:'middle',margin:'-3px 5px 0 0'}}/>{title}</Err>
            </ErrWrapper>
    ) || null
}